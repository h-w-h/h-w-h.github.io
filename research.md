...
Let \\(\mathcal{H}\\) be a Hilbert space of functions  \\(u:\Omega\rightarrow\mathbb{R}\\), where \\(\Omega\subset\mathbb{R}^n\\). Variational methods define energy functionals for which a minimizer \\(u\\) possesses desirable properties, e.g., \\(u\\) is a denoised version of a blurry and grainy image \\(f\\). The classic Rudin-Osher-Fatemi (ROF) model proposes using the energy  \\((J(u)\\) to accomplish this task, which is defined by
