...
---
 ...
 where \\( \lbrace C_i\rbrace_{i=1}^m \\) forms a finite family of closed convex sets with nonempty intersection. On the other hand, variational models are used in many image processing methods, e.g., the Rudin-Osher-Fatemi (ROF) model aims to minimize the "energy" \\(E(u)\\) defined by
