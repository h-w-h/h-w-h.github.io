...
This means, in practice, a processor can carry out its computations using whatever iterate \\(x^k\\) it received most recently (rather than remain idle waiting for the slowest processor to catch up). With appropriate assumptions on the delays and step-sizes used, asynchronous updates generate a sequence that converges a feasible point in \\(C\\). 
