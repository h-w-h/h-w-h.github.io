...
Asynchronous algorithms are of great practical interest as they open the door to greater utilization of processing power than anologous synchronous algorithms. Let \\(\tau\)\\ be a nonnegative integer, identifying how out-of-date of information we wish for processors to be able to utilize. For synchronous methods, \\(\tau =0\\), but for asynchronous methods we often take \\(\tau > 0\\). As it relates to convex feasibility problems, asynchronous methods replace the \\(x^k\\) in the update above with \\( \hat{x}^k = x^{k-j} \\), which is a possibly out-of-date iterate, where \\(j \in \lbrace 0, 1, 2, \ldots, \tau \rbrace \\) for some nonnegative integer \\(\tau\\). In mathematical terms, we write
