...
In practice, we take this to mean a processor can carry out its computations using whatever iterate \\(x^k\\) it received most recently (rather than remain idle waiting for the slowest processor to catch up). With appropriate assumptions, this new sequence also converges. This modification enables various processors to use out-of-date information and, therefore, continue computations  
